...
export async function render(params, root) {
  ...
  root.innerHTML = `
    ...
    <p style="color:var(--ink-mute);font-size:1rem;margin:.4rem 0 .5rem;">${tms.length} TMs · single-copy locations and infinite-copy locations</p>
  ...
  function apply() {
    ...
    tblEl.innerHTML = `
      <table class="table">
        <thead><tr><th>#</th><th>Move</th><th>Type</th><th class="num">Pow</th><th class="num">Acc</th><th>Single Copy</th><th>Infinite</th></tr></thead>
        <tbody>
          ${result.map((t) => {
